    ...
    byte* keyData, word32 keySize, int *key_found)
{
    ...
    if ((i + 1) < keySize && keyData[i] == 0xa2) {
        ...
        if (wolfssl_der_get_length(keyData, &i, keySize, &len) < 0)
            return;
        ctx->spki.salt_size = 0;
        while (len-- > 0) {
             ctx->spki.salt_size <<= 8;
             ctx->spki.salt_size += keyData[i++];
        }
    }
