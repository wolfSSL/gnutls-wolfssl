...
const unsigned char expected_no_data_aes_cmac_128[] = {
    0x25, 0x73, 0x14, 0x45, 0xfd, 0xfe, 0xeb, 0xc3,
    0x47, 0x2b, 0xb0, 0x3b, 0xf7, 0x0a, 0x91, 0x3f
};
const unsigned char expected_aes_cmac_256[] = {
    0xbe, 0x00, 0x87, 0x3f, 0x2f, 0x7f, 0x59, 0x08,
    0xbb, 0x21, 0x12, 0x63, 0x28, 0x29, 0x62, 0x94
};
const unsigned char expected_no_data_aes_cmac_256[] = {
    0x2e, 0xb4, 0x36, 0x68, 0x4a, 0xe1, 0x60, 0xec,
    0x8e, 0x5c, 0x03, 0xad, 0x8e, 0x03, 0x9a, 0x6a
};


static int test_cmac(int algorithm, size_t key_sz,
    const unsigned char* expected, size_t sz,
    const unsigned char* expected_no_data)
{
    int ret;
    ...
    /* Use the same key and no data to hash. */
    gnutls_hmac_output(cmac, output);
    if (compare("CMAC", output, expected_no_data, sz) != 0) {
        gnutls_hmac_deinit(cmac, NULL);
        return 1;
    }
...
int main(void)
{
    ...
    ret = test_cmac(GNUTLS_MAC_AES_CMAC_128, 16, expected_aes_cmac_128,
        sizeof(expected_aes_cmac_128), expected_no_data_aes_cmac_128);
    if (ret == 0) {
        ...
        ret = test_cmac(GNUTLS_MAC_AES_CMAC_256, 32, expected_aes_cmac_256,
            sizeof(expected_aes_cmac_128), expected_no_data_aes_cmac_256);
    }
